...
// @Controller + RequestMapping이랑 합친 것
// 서블릿에서는 메서드 단위로 맵핑이 안되고, 클래스 단위로만 된다. 그래서 서블릿은 스프링에 비해 클래스를 많이 만들어야하고,
// HttpServlet을 상속 받아야하고 메서드 이름은 service로 해주고 매개변수를 HttpServletRequest, HttpServletResponse 2가지를 써야함
...
public class MyDispatcherServlet extends HttpServlet {
	...
	public void service(HttpServletRequest request, HttpServletResponse response) throws IOException {
		Map map = request.getParameterMap(); // year=2021&month=10&day=1가 key/value로 map에 들어온다.
		Model model = null;
		String viewName = "";
		
		try {
			Class clazz = Class.forName("com.fastcampus.ch2.YoilTellerMVC"); //YoilTellerMVC 객체 생성
