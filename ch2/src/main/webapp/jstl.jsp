...
<body>
<c:set var="to"   value="10"/> <!-- 저장소에 저장 [key:"to" / value:"10" 이런 식으로 -->
...
<c:if test="${not empty arr}">
	<c:forEach var="elem" items="${arr}" varStatus="status">
		${status.count}. arr[${status.index}]=${elem}<BR> <!-- count는 1부터 시작 / index는 0부터 시작 ex) 1.arr[0]=10 -->
	</c:forEach>
</c:if>	
<c:if test="${param.msg != null}">
	msg=${param.msg} 
	msg=<c:out value="${param.msg}"/> , <!-- script 공격 방어 -->
</c:if>
<br>
